...
def multiply_factors(f1: pd.DataFrame, f2: pd.DataFrame) -> pd.DataFrame:
    ...
    if not common_vars:
        ...
        f_res = pd.merge(f1.reset_index(), f2.reset_index(), on=common_vars, how="inner")            .set_index(keys=disjoint_vars + common_vars)
        f_res["prob"] = f_res["prob_x"] * f_res["prob_y"]
        f_res = f_res.drop(columns=["prob_x", "prob_y"])
...
if __name__ == "__main__":
    bn = BayesNet(bn_file="bnet")
    jt = JunctionTree(bn)
    nx.draw(jt.clique_tree)
